...
let initInterpreter _ =
  ...
  let execute content =
    ...
    Printf.printf "execute %s" content;
    let content' =
...
let execute code =
  ...
  Buffer.add_buffer combined_buffer err_buffer; *)
...
let executeJS =
  Js.wrap_callback
      (fun _code ->
        try
          execute _code
          (* Lwt.catch(fun () -> Lwt.return (execute _code))
          (fun _exn -> Lwt.return (Js.string "Lwt Error")) *)
          (* failwith "TestError" *)
      with
      | Js_error.Exn e -> Js_error.raise_ e
      (* | Failure s -> Js.raise_js_error (new%js Js.error_constr (Js.string s)) *)
      | Failure s -> Js_error.raise_ (Js_error.of_error (new%js Js.error_constr (Js.string s)))
      (* | Failure _ -> Js_error.raise_ (Js_error.of_error (new%js Js.error_constr (Js.string "Test"))) *)
      (* | _ -> Js_error.raise_ (Js_error.of_error (new%js Js.error_constr (Js.string "unknown error"))) *)
      (* | _ -> Js.string "unknown Error" *)
      (* | e -> match Js_error.of_exn e with
        (* | Some e -> Js_error.raise_ e *)
        | Some e -> Js_error.raise_ e
...
let () =
  Js.export "evaluator" (
    object%js
      method init x = initInterpreter x
      method reset x = resetJS x
